...
data "terraform_remote_state" "state" {
  ...
  config = {
    ...
    region         = "${var.region}"
    key            = "${var.serverless_application_name}/${var.environment}"
    encrypt        = true
  }
}
...
module "lambda_functions" {
  ...
  environment_variables = {
    ...
    USER_POOL_ID             = "${var.cognito_stack_name}-UserPool"
    APPLICATION_CLIENT_ID    = "${var.cognito_stack_name}-UserPoolClient"
    ADMIN_GROUP_NAME         = "${var.cognito_stack_name}-UserPoolAdminGroupName"
  }
  ...
  allowed_triggers = {
    APIGatewayAny = {
      service    = "apigateway"
      source_arn = aws_api_gateway_rest_api.application_api.arn
    }
  }
}
